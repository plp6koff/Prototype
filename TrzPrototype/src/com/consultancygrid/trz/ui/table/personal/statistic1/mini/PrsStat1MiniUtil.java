...
public class PrsStat1MiniUtil {
	...
	public static void load(EntityManager em,
		...
		String queryNative = "SELECT 1,MATCHCODE, V18, V15,  V02,  V01,  V19, ZAPLATA_V_BROI, V14, CODE "
				+ " FROM EMPLOYEE_STATS  "
				+ " where 1=1 and ( MATCHCODE=:P_MATCHCODE or :P_MATCHCODE is null)  and ( CODE like :P_PERIODCODE||'%' or :P_PERIODCODE is null) "
				+ " union all "
				+ " SELECT 2,MATCHCODE, sum(V18),sum(V15),sum(V02),sum(V01), sum(V19),sum(ZAPLATA_V_BROI),sum(V14) , null"
				+ " FROM EMPLOYEE_STATS  "
				+ " where 1=1  and ( MATCHCODE=:P_MATCHCODE or :P_MATCHCODE is null)  and ( CODE like :P_PERIODCODE||'%' or :P_PERIODCODE is null) "
		...
		for (Object o : allRows) {
			...
			int i = 1;
			boolean boldRow = (null == currentRow[9] || " ".equals(currentRow[9]) || "N/A".equals(currentRow[9]));
			...
			Object zaplt = (currentRow[7] == null ? "0.0" : currentRow[7]);
			BigDecimal zapltBD = null;
			if (zaplt instanceof BigDecimal) {
			  zapltBD = (BigDecimal) zaplt;
			} else if (zaplt instanceof String) {
				zapltBD = BigDecimal.valueOf(Double.valueOf((String)zaplt));
			}
			...
			setBoldData(oneRow, boldRow, zapltBD.subtract(avansBD).subtract(vaucherBD));
