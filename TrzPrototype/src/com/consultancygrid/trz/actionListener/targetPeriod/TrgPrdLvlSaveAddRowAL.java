...
public class TrgPrdLvlSaveAddRowAL extends BaseActionListener {
	...
	public TrgPrdLvlSaveAddRowAL(PrototypeMainFrame mainFrame,
			TargetLevelCfgTable tLvlConfTable, JComboBox comboBoxTargetPeriod,
	...
	public void actionPerformed(ActionEvent e) {
		...
		try {
			...
			TargetPeriod targetPeriod = (TargetPeriod) comboBoxTargetPeriod
					.getSelectedItem();

			TargetLevelCfgTableModel model = (TargetLevelCfgTableModel) tLvlConfTable
					.getModel();

			if (targetPeriod != null) {

				TargetLevels tl = new TargetLevels();
				String percent = map.get("PERCENT").getText();
				String bonus = map.get("BONUS").getText();
				tl.setTargetPercent(BigDecimal.valueOf(Double.valueOf(percent)));
				tl.setTargetBonus(BigDecimal.valueOf(Double.valueOf(bonus)));
				tl.setTargetPeriod(targetPeriod);
				em.persist(tl);

				Query q1 = em
						.createQuery(" from TargetLevels as tl  where  tl.targetPeriod.id = :targetId");
				q1.setParameter("targetId", targetPeriod.getId());

				List<TargetLevels> allTPLs = (List<TargetLevels>) q1
						.getResultList();
