...
public class TrgPrd2EmplSaveAL extends BaseActionListener {

	
	private static Logger logger = Logger.getLogger(TrgPrd2EmplSaveAL.class);
	...
	public TrgPrd2EmplSaveAL(PrototypeMainFrame mainFrame, JTextField value,
			JComboBox comboBoxTarget, JComboBox comboBoxEmployee,
			JPanel createPanel, TargetEmplPeriod targEmplPer, boolean update,
			JButton button) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		try {
			...
			Employee empl = (Employee) comboBoxEmployee.getModel()
					.getSelectedItem();
			if (empl == null) {
			...
			if (valueTarget == null || "".equals(valueTarget)
					|| valueTarget.length() > 7) {
				JOptionPane.showMessageDialog(mainFrame,
						"Please fill the value !", ResourceLoaderUtil
								.getLabels(LabelsConstants.ALERT_MSG_ERR),
						JOptionPane.ERROR_MESSAGE);
				return;
			}
			...
			TargetPeriod tP = (TargetPeriod) comboBoxTarget.getModel()
					.getSelectedItem();

			targEmplPer.setTargetPeriod(tP);
			targEmplPer.setEmployee(empl);
			targEmplPer.setTargetValue(BigDecimal.valueOf(Double.valueOf(value
					.getText())));
			...
			try {
				JOptionPane.showMessageDialog(mainFrame,
						"Target 'Linked to employee ", ResourceLoaderUtil
								.getLabels(LabelsConstants.ALERT_MSG_INFO),
			...
			try {
				JOptionPane.showMessageDialog(mainFrame, ResourceLoaderUtil
						.getLabels(LabelsConstants.SET_TAB_CRT_PERIOD_ERR),
						ResourceLoaderUtil
								.getLabels(LabelsConstants.ALERT_MSG_ERR),
						JOptionPane.ERROR_MESSAGE);
			} catch (HeadlessException | IOException e2) {
