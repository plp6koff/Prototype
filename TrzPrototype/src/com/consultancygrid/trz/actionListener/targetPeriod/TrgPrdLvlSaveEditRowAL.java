...
public class TrgPrdLvlSaveEditRowAL extends BaseActionListener{
	...
	public TrgPrdLvlSaveEditRowAL(PrototypeMainFrame mainFrame, TargetLevelCfgTable tLvlConfTable, 
		...
		try {
			...
			if (targetPeriod != null) {
				...
				if (allTargetLevels != null && !allTargetLevels.isEmpty()) {
					...
					em.merge(tl);
					//((TargetLevelCfgTableModel) model).setValueAt(percent, i, 1);
					//((TargetLevelCfgTableModel) model).setValueAt(bonus, i, 2);
					//((TargetLevelCfgTableModel) model).fireTableRowsUpdated(i, i);
