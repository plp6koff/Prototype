...
public class GroupTablPeriodLoaderUtil {
	...
	public Set<UUID> loadData(Period period, EntityManager em, Vector tableData)
		...
		Query q = em
				.createQuery(" select emplDeptP from EmplDeptPeriod as emplDeptP "
		...
		List<EmplDeptPeriod> emplsDepartments = (List<EmplDeptPeriod>) q
				.getResultList();
		Query qAllEmployees = em
				.createQuery("select count(empl.id) from Employee empl  ");
		...
		for (EmplDeptPeriod emplDeptPeriod : emplsDepartments) {
			...
			if ((period.getId() != null)
					&& period.getId()
							.equals(emplDeptPeriod.getPeriod().getId())) {

				Employee employee = emplDeptPeriod.getEmployee();
				EmployeeSettings emplSettings = null;
				if (employee.getEmployeeSettingses() != null
						&& employee.getEmployeeSettingses().size() > 0) {
				...
				add1thRow(tableData, employee,
						...
						emplBonus, emplSettings, bonusAll);
				final double bonusGroup =  ((BigDecimal)objcts[1]).doubleValue();
				add2thRow(tableData, employee, department, depBonus, emplBonus, emplSettings, deptPerns, bonusGroup);
				final double bonusPersonal =    ((BigDecimal)objcts[0]).doubleValue();
				add3thRow(tableData, employee,
						department, emplBonus, percentPersonal, bonusPersonal);
				...
				Query sq = em
						.createQuery(" from EmployeeSalary as sal  where  sal.period.id = :periodId and sal.employee.id = :emplId");
				sq.setParameter("periodId", period.getId());
				sq.setParameter("emplId", employee.getId());
				EmployeeSalary salary = ((List<EmployeeSalary>) sq
						.getResultList()).get(0);

				salary.setV06(BigDecimal.valueOf(bonusAll).setScale(2,
						BigDecimal.ROUND_HALF_UP));
				salary.setV07(BigDecimal.valueOf(bonusGroup).setScale(2,
						BigDecimal.ROUND_HALF_UP));
				salary.setV08(BigDecimal.valueOf(bonusPersonal).setScale(2,
						BigDecimal.ROUND_HALF_UP));
				salary.setV09(BigDecimal.valueOf(
						bonusAll + bonusGroup + bonusPersonal).setScale(2,
						BigDecimal.ROUND_HALF_UP));
				em.merge(salary);
			}
		}
	...
	private boolean checkIsPropertyChanged(BigDecimal property,
			BigDecimal oldVal) {
		return property.compareTo(oldVal) != 0;
	}

	public void loadData2(Period period, EntityManager em, Vector tableData)
			throws IOException {

		Query q = em
				.createQuery(" select empl.id from EmplDeptPeriod as emplDeptP "
						+ " join emplDeptP.period  as period "
						+ " join emplDeptP.department as department "
						+ " join  emplDeptP.employee as empl"
		...
		for (EmployeeSettings emplSettings : allSettings) {
			...
			final BigInteger emplBonus = getEmployeeRevenue(em, period,
					employee);
			final double bonusAll = add1thRow(tableData, employee, period,
					emplBonus, emplSettings);
			final double bonusGroup = add2thRow(tableData, employee, emplBonus,
					emplSettings);
			final double bonusPersonal = add3thRow(tableData, employee,
					emplBonus, percentPersonal);
			final double totalBonus = bonusAll + bonusGroup + bonusPersonal;
			final double totalPercent = (totalBonus * 100)
					/ (emplBonus.intValue() == 0 ? 1 : emplBonus.intValue());
			...
			sq.setParameter("periodId", period.getId());
			sq.setParameter("emplId", employee.getId());
			EmployeeSalary salary = ((List<EmployeeSalary>) sq.getResultList())
					.get(0);
			salary.setV06(BigDecimal.valueOf(bonusAll).setScale(2,
					BigDecimal.ROUND_HALF_UP));
	...
	private BigInteger getEmployeeRevenue(EntityManager em, Period period,
			Employee employee) {
		Query tempPerEmpl = em
				.createQuery(" from RevenueEmplPeriod as revEmplPer  where  revEmplPer.period.id = :periodId and revEmplPer.employee.id = :emplId ");
		tempPerEmpl.setParameter("periodId", period.getId());
		tempPerEmpl.setParameter("emplId", employee.getId());
		List<RevenueEmplPeriod> revenueEmplPeriods = (List<RevenueEmplPeriod>) tempPerEmpl
				.getResultList();
	...
	private BigInteger getDepartmentRevenue(EntityManager em, Period period,
	...
	private Double getEmployeePercentAll(EmployeeSettings emplSettings) {
		if (emplSettings != null) {
			BigDecimal temp = emplSettings.getPercentAll() != null ? emplSettings
					.getPercentAll() : BigDecimal.valueOf(1);
			return temp.setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
		} else {
			// TODO defailt value
			return BigDecimal.valueOf(1).setScale(2, BigDecimal.ROUND_HALF_UP)
					.doubleValue();
		}
	}
	...
	private Double getEmployeePercentGroup(EmployeeSettings emplSettings) {
		if (emplSettings != null) {
			BigDecimal temp = emplSettings.getPercentGroup() != null ? emplSettings
					.getPercentGroup() : BigDecimal.valueOf(1);
			return temp.setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
		} else {
			// TODO defailt value
			return BigDecimal.valueOf(1).setScale(2, BigDecimal.ROUND_HALF_UP)
	...
	private Double getEmployeePercentPersonal(EmployeeSettings emplSettings) {
		if (emplSettings != null) {
			BigDecimal temp = emplSettings.getPercentPersonal() != null ? emplSettings
					.getPercentPersonal() : BigDecimal.valueOf(1);
			return temp.setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
		} else {
			// TODO defailt value
			return BigDecimal.valueOf(1).setScale(2, BigDecimal.ROUND_HALF_UP)
	...
	public Double getEmployeePercentAllOnboard(EmployeeSettings emplSettings) {
		if (emplSettings != null) {
			BigDecimal temp = emplSettings.getPersonAllOnboardingPercent() != null ? emplSettings
					.getPersonAllOnboardingPercent() : BigDecimal.valueOf(1);
			return temp.setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
		} else {
			// TODO defailt value
			return BigDecimal.valueOf(1).setScale(2, BigDecimal.ROUND_HALF_UP)
	...
	private Double getEmployeePercentGroupOnboard(EmployeeSettings emplSettings) {
		if (emplSettings != null) {
			BigDecimal temp = emplSettings.getPersonGroupOnboardingPercent() != null ? emplSettings
					.getPersonGroupOnboardingPercent() : BigDecimal.valueOf(1);
			return temp.setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
		} else {
			// TODO defailt value
			return BigDecimal.valueOf(1).setScale(2, BigDecimal.ROUND_HALF_UP)
	 ...
	 * @throws IOException
	 */
	private void add1thRow(Vector tableData, Employee employee,
			Department department, int allEmployeesCount, Period period,
			BigInteger emplDept, EmployeeSettings employeeSettings, double bonusAll)
			throws IOException {

		Vector<Object> oneRow = new Vector<Object>();
		// a0
		String departmetnCode = department != null ? department.getCode()
				: "N/A";
		...
		//double result = calculateBonusAll(profitAll, percentAll,
		//		personalFactor, jobDonePercent, allEmployeesCount);
		//oneRow.add(result);
		oneRow.add(bonusAll);
	...
	private Vector add1thRow(Employee employee, Department department,
			int allEmployeesCount, Period period, BigInteger emplDept,
	...
	private double add1thRow(Vector tableData, Employee employee,
			Period period, BigInteger emplDept,
			EmployeeSettings employeeSettings) throws IOException {
		...
		// TODO last 2 columns to be token from the Excel
		double result = calculateBonusAll(profitAll, percentAll,
				personalFactor, jobDonePercent, 1);
		oneRow.add(result);
		oneRow.add(calculateTotalAll(period.getRevenue(), percentAll));
		tableData.add(oneRow);
		return result;
	...
	private Double calculateTotalAll(BigDecimal revenuPeriond, Double percentAll) {
		...
		return BigDecimal
				.valueOf((revenuPeriond.doubleValue() * percentAll) / 100)
				.setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
	}

	public Double calculateBonusAll(int profitAll, double percentAll,
			double personalFactor, double jobDonePercent, int allEployees) {

		return BigDecimal
				.valueOf(
						profitAll
								* (((percentAll * personalFactor) / allEployees) / (100 * jobDonePercent)))
				.setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
	}

	private void add2thRow(Vector tableData, Employee employee,
			Department department, BigInteger revenueDept, BigInteger emplDept,
			EmployeeSettings employeeSettings, Map<String, Long> deptPernsCountMap, double bonusGroup) throws IOException {
		Vector<Object> oneRow = new Vector<Object>();
		...
		double jobDonePercent = 1.0d;
		// TODO last 2 columns to be token from the Excel
//		double result = calculateBonusGroup(profitGroup, percentGroup,
//				personalFactor, jobDonePercent, allEmployeesDept);
//		oneRow.add(result);
		oneRow.add(bonusGroup);
		oneRow.add(calculateTotalGroup(
				revenueDept.intValue() - emplDept.intValue(), percentGroup));
		tableData.add(oneRow);
	}

	private double add2thRow(Vector tableData, Employee employee,
			BigInteger emplDept, EmployeeSettings employeeSettings)
			throws IOException {
		Vector<Object> oneRow = new Vector<Object>();
		oneRow.add("N/A");
		oneRow.add(EMPTY_STRING);
	...
	private void add3thRow(Vector tableData, Employee employee,
			Department department, BigInteger revenueEmp, Double percentPersonal, double bonusPersonal)
			throws IOException {
		Vector<Object> oneRow = new Vector<Object>();
		oneRow.add(department.getCode());
		oneRow.add(EMPTY_STRING);
		...
		// oneRow.add("1");
		// TODO last 2 columns to be token from the Excel
//		double result = calculateBonusPersonal(profitPersonal,
//				percentPersonal.doubleValue(), personalFactor, jobDonePercent);
//		oneRow.add(result);
		oneRow.add(bonusPersonal);
		oneRow.add(calculateTotalPersonal(1, percentPersonal));
		tableData.add(oneRow);
	}

	private double add3thRow(Vector tableData, Employee employee,
			BigInteger revenueEmp, Double percentPersonal) throws IOException {
		Vector<Object> oneRow = new Vector<Object>();
		oneRow.add("N/A");
		oneRow.add(EMPTY_STRING);
		oneRow.add(EMPTY_STRING);// To be extracted from db
	...
	public Double calculateTotalPersonal(int base, Double percentPersonal) {
		return BigDecimal.valueOf((base * percentPersonal) / 100)
				.setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
	}

	public Double calculateBonusPersonal(double profitPersonal,
			double percentPersonal, double personalFactor, double jobDonePercent) {

		return BigDecimal
				.valueOf(
						profitPersonal
								* ((percentPersonal * personalFactor) / (100 * jobDonePercent)))
				.setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
	}
